...
def show_appointments():
    ...
    for appointment in appointments:
        ...
        for expense in appointment["expenses"]:
            print(
                f"  - 일자: {expense['date'].strftime('%Y-%m-%d')}, 설명: {expense['description']}, 금액: {expense['amount']}")
...
def import_account():
    ...
    try:
        ...
        new_account.income_total = account_data['history'][
            0]  # 단순 history에서 Account_book 안의 멤버 변수들로 교체 ->  new_account 객체의 멤버 변수들에 저장
