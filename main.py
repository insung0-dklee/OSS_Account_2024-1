...
def view_entries():
    for entry in ledger:
        ...
        print(f"날짜: {entry['date']}, 카테고리: {entry['category']}, 설명: {entry['description']}, 금액: {amount_display}원, 유형: {entry['type']}")
        if "score" in entry:
...
def set_budget():
    ...
    current_income_total = sum(float(entry["amount"]) for entry in ledger if entry["type"] == "수입")  # 수입 항목 합산
    ...
    # 수입과 지출 내역 출력
    print(f"현재 수입: {current_income_total} 원")
