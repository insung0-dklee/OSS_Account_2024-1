...
>>>>>>> 6b1b256d5ee7a36c097b140da536b4e1fc9b98fc
...
#현금 수입 기록
def record_income(amount, desc):
    global cash_balance
    cash_balance += amount
    income_list.append((amount, desc))
    print(f"{desc} - {amount}원 수입 기록되었습니다. 현재 잔액: {cash_balance}원")

#현금 지출 기록
def record_expense(amount, desc):
    global cash_balance
    if cash_balance >= amount:
        cash_balance -= amount
        expense_list.append((amount, desc))
        print(f"{desc} - {amount}원 지출 기록되었습니다. 현재 잔액: {cash_balance}원")
    else:
        print(f"잔액이 부족합니다. 현재 잔액: {cash_balance}원")

#현금 잔액 조회
def show_records():
    print("현금 수입 내역:")
    for amount, desc in income_list:
        print(f"{desc} - {amount}원")
    print("\n현금 지출 내역:")
    for amount, desc in expense_list:
        print(f"{desc} - {amount}원")
...
=======
        ...
        analyze_categories()
    elif func == "6": # 현금 기록 기능 추가
        cash_func = input("1. 현금 입금, 2. 현금 출금, 3. 내역 보기: ")
        if cash_func == "1":
            amount = int(input("입금할 금액 입력: "))
            desc = input("입금 내역 설명: ")
            record_income(amount, desc)
        elif cash_func == "2":
            amount = int(input("출금할 금액 입력: "))
            desc = input("출금 내역 설명: ")
            record_expense(amount, desc)
        elif cash_func == "3":
            show_records()
        else:
            print("올바른 메뉴를 선택하세요.")
