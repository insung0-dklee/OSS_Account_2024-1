...
def quiz(): #생활 경제 퀴즈
    ...
    questions = [
        {
            "question": "이자의 기준은 OO이다. 이것이 높아지면 이자도 늘어난다. OO의 답은?\n1. 금리\n2. 화폐\n3. 금융",
            "answer": "1"
        },
        {
            "question": "은행에 예금하면 대가로 받는 것은 금리이다. (O/X 문제)",
            "answer": "O"
        },
        {
            "question": "금리는 OO에서 차지하는 이자의 비율이다. OO는 무엇인가?\n1. 원금\n2. 이자\n3. 대출",
            "answer": "1"
        }
    ] #questions에 세 개의 퀴즈를 저장

    selected_question = random.choice(questions) #랜덤으로 세 가지의 퀴즈 중 하나 선택
    print(selected_question["question"]) #해당 퀴즈 출력
    
    user_answer = input("답을 입력하세요: ")#퀴즈에 해당하는 정답을 입력
    
    if user_answer == selected_question["answer"]: #퀴즈에 해당하는 정답이 맞다면 정답, 틀리다면 오답을 출력
        print("정답입니다!")
    else:
        print("오답입니다.")
