...
"""
지출/수입 내역 검색 기능
사용자로부터 검색할 키워드를 입력받음
ledger 내의 각 항목에 대해 키워드가 포함되어 있는지 확인
검색 결과가 있으면 출력, 없으면 메시지 출력
...
def search_entries():
    ...
    results = []
    for entry in ledger:
        if keyword in entry["description"] or keyword in entry["category"]:
            results.append(entry)
    if results:
        for result in results:
            print(result)
    else:
        print("해당 키워드가 포함된 내역이 없습니다.")

"""
잔고 계산 기능
현재 잔고 출력
잔고 계산 (수입 - 지출)
ledger 내의 모든 지출 합계 계산
ledger 내의 모든 수입 합계 계산
"""
def calculate_balance():
    total_income = sum(entry["amount"] for entry in ledger if entry["amount"] > 0)
